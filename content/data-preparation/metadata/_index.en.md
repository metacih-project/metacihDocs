...
Next to the actual data file, each MetaCIH repository of an evidence collection also contains a **folder** in which **metadata** is stored. Metadata items may vary between repositories, but a core selection of files is always included. We focus on these metadata items here.
...
* 📄 [**`references.json`**](https://github.com/metapsy-project/data-template/blob/master/metadata/references.json). A [`.json`](https://www.json.org/json-en.html) file, containing references of the included studies. 
...
* 📄 [**`shorthand.txt`**](https://github.com/metapsy-project/data-template/blob/master/metadata/shorthand.txt): A `.txt` file with the `metapsyData` [shorthand](/databases/#shorthand) of the database.
